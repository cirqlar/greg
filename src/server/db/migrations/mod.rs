...
mod tests {
    ...
    async fn v1_db(#[future(awt)] empty_db: Database) -> Connection {
        let v1_db = empty_db.connect().expect("Can connect");
        fill_db(&v1_db, "tests/dumps/v1_dump.sql").await;
    ...
    #[fixture]
    async fn v2_db(#[future(awt)] empty_db: Database) -> Connection {
        let v2_db = empty_db.connect().expect("Can connect");
        fill_db(&v2_db, "tests/dumps/v2_dump.sql").await;
